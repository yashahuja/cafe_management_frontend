...
import { AqiParameter } from './charts.data';
...
export class ChartsComponent implements OnInit {
    ...
    this.filteredOptions = this.myControl.valueChanges.pipe(
      ...
      map((value: any) => {
        ...
        return lat && lon && address
          ? this._filter({ lat: lat, lon: lon, address: address })
          : this.latLongList.slice();
      })
    );
  }

  public prepareLatLongList() {
    this.ChartsService.getData(AppSettings.GetUniqueCoordinates).subscribe(
      (res: any) => {
        if (res && res.dataList) {
  ...
  displayFn(coords: any): string {
    return coords && coords.lat && coords.lon && coords.address ? coords : '';
  }
  ...
  private _filter(coords: any): any[] {
    ...
    return this.latLongList.filter(
      (item) =>
        (item.lat && item.lat.toString().includes(lat)) ||
        (item.lon && item.lon.toString().includes(lon)) ||
        (item.address &&
          item.address
            .toString()
            .toLowerCase()
  ...
  public createChart() {
    ...
    this.ChartsService.getData(AppSettings.GetChartValues, {
      lat: lat,
      lon: lon,
      fromDate: sdate,
      toDate: edate,
